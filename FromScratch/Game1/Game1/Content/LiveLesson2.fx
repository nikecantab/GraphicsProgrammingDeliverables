...
float4 MainPS(VertexShaderOutput input) : COLOR
{
    ...
    float3 light = max(dot(perturbedNormal, -lightDirection) * LightColor, 0.0);

    float3 litTexture = (light + spec * Specularity) * texColor.rgb;

    float3 ambientTexture = AmbientLight * texColor.rgb;

    return float4(saturate(litTexture + ambientTexture),1);
}
